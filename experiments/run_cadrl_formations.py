...
import maca.envs.test_cases as tc
from maca.env_utils import run_episode, create_env, store_stats, policy_dict



def reset_env(env, one_env, test_case_fn, test_case_args, test_case, num_agents, policies, policy,
              prev_agents=None, start_from_last_configuration=True):
    if prev_agents is None:
        prev_agents = tc.small_test_suite(num_agents=num_agents,
                                          test_case_index=0,
                                          policies=policies[policy]['policy'],
                                          agents_sensors=policies[policy]['sensors'])
