...
from maca.policies.CADRL.scripts.multi import global_var as gb
...
def generate_rand_test_case_multi(num_agents, side_length, speed_bnds, radius_bnds, is_end_near_bnd = False, is_static = False):
    ...
    if is_static:
        test_case = generate_static_case(num_agents_sampled, side_length, speed_bnds, radius_bnds)
    # else:
    # 	if random_case < 0.5:
    ...
    # 			speed_bnds, radius_bnds)
        ...
        # print(random_case)
        if random_case < 0.15:
            test_case = generate_swap_case(num_agents_sampled, side_length, speed_bnds, radius_bnds)
        elif random_case > 0.15 and random_case < 0.3:
            test_case = generate_circle_case(num_agents_sampled, side_length, speed_bnds, radius_bnds)
        else:
            # is_static == False:
...
def generate_rand_case(num_agents, side_length, speed_bnds, radius_bnds, is_end_near_bnd=False):
    ...
    for i in range(num_agents):
        ...
        # generate random starting/ending points
            ...
            if is_end_near_bnd == True:
                ...
                if random_side == 0:
                    end[0] = np.random.rand() * 0.1 * side_length - side_length
                elif random_side == 1:
                    end[0] = np.random.rand() * 0.1 * side_length + 0.9 * side_length
                elif random_side == 2:
                    end[1] = np.random.rand() * 0.1 * side_length - side_length
                elif random_side == 3:
                    end[1] = np.random.rand() * 0.1 * side_length + 0.9 * side_length
                else:
                    assert(0)
