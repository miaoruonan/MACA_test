...
from maca.envs import test_cases as tc
from maca.envs.util import l2norm
from maca.envs.visualize import plot_episode, animate_episode

class ObCollisionAvoidanceEnv(gym.Env):
    ...
    def get_neighbor(self, id):
        ...
        Adjacency matrix needs to be set
        :return:
        """
        # neighbor_data = []
        if id > len(self.agents):
            print("The Id is error!")
            return False

        # for i in range(neighbor.shape[1]):
        #     if neighbor[id][i] == 1:
        #         neighbor_data.append(self.agents[i])
