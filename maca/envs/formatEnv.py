...
from maca.envs.Map import Map
from maca.envs.util import find_nearest, rgba2rgb, l2norm
from maca.envs.visualize import plot_episode, animate_episode

class FormationEnv(gym.Env):
    """ Gym Environment for multiagent decision making and Control in Formation
    ...
    def reset(self):
        ...
        if self.episode_step_number is not None and self.episode_step_number > 0 and self.plot_episodes and self.test_case_index >= 0:
            plot_episode(self.agents, self.evaluate, self.map, self.test_case_index, self.id, circles_along_traj=Config.PLOT_CIRCLES_ALONG_TRAJ,
                         plot_save_dir=self.plot_save_dir, plot_policy_name=self.plot_policy_name, limits=self.plt_limits, fig_size=self.plt_fig_size,
                         show=Config.SHOW_EPISODE_PLOTS, save=Config.SAVE_EPISODE_PLOTS)
            if Config.ANIMATE_EPISODES:
                animate_episode(num_agents=len(self.agents), plot_save_dir=self.plot_save_dir, plot_policy_name=self.plot_policy_name,
                                test_case_index=self.test_case_index, agents=self.agents)
    ...
    def _compute_rewards(self):
        ...
        collision_with_agent, collision_with_wall, entered_norm_zone, dist_btwn_nearest_agent = self._check_for_collisions()
