...
class Obstacle(BaseAgent):
    ...
    """
    def __init__(self, start_x, start_y, radius, id, sensors,neighbor_ids, neighbor_pos, neighbor_info, dynamics_model):

        # Store past selected actions
        self.chosen_action_dict = {}

        # self.neighbor_ids = np.array([1,3])
        # self.neighbor_pos = np.array([[1, 0], [0, -1]])
        # self.neighbor_info = {1: [1, 0], 3: [0, -1]}
        ...
        self.ref_prll = np.array([0,0])
        self.ref_orth = np.array([0,0])
        self.reset(px=start_x, py=start_y, gx=start_x, gy= start_y, radius=radius, heading=self.initial_heading)

    def reset(self, px=None, py=None, gx=None, gy=None, radius=None, heading=None):
...
if __name__ == '__main__':
    ...
    sensors = []
    id = 0
    agent = DistributedAgent(start_x=start_x, start_y=start_y,
                             radius=radius, initial_heading=initial_heading, dynamics_model=FullDynamics, id=id)
